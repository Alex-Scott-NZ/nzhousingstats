  ...
  async () => {
    ...
    try {
      ...
      if (buyResult.success) {
        console.log(
          `✅ HOUSES_TO_BUY: ${buyResult.totalNzListings.toLocaleString()} listings across ${buyResult.totalRecords.toLocaleString()} locations stored`
        );

        // Add git sync after successful data collection
        console.log("📤 Syncing database to GitHub...");
        try {
          const { execSync } = await import("child_process");

          // Add and commit the updated database
          execSync("git add data/nzhousingstats.db", { cwd: process.cwd() });
          execSync(
            `git commit -m "Daily data update: ${buyResult.totalNzListings.toLocaleString()} listings - ${
          ...
          console.error("❌ Git sync failed:", gitError.message);
        }
      } else {
      ...
      console.log(
        `🎉 Daily data collection completed at ${new Date().toISOString()}`
      );
      console.log(
        `📈 Next collection scheduled for tomorrow at 5:00 PM NZ time\n`
      );
    } catch (error) {

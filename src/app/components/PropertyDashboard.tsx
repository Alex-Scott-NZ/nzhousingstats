...
export default function PropertyDashboard({ allData, snapshots }: PropertyDashboardProps) {
  const [selectedListingType, setSelectedListingType] = useState<HouseListingType>('HOUSES_TO_BUY');
  ...
  const availableRegions = useMemo((): DropdownOption[] => {
    ...
    return currentListingData.regions
      ...
      .sort((a: DropdownOption, b: DropdownOption) => a.name.localeCompare(b.name));
  }, [currentListingData]);
  ...
  const availableDistricts = useMemo((): DropdownOption[] => {
    ...
    return currentListingData.districts
      .filter((district: DatabaseLocationSnapshot) => district.regionId === selectedRegionId)
  ...
  const currentDisplayData = useMemo((): DatabaseLocationSnapshot[] => {
    ...
    if (selectedDistrictId) {
      ...
      return currentListingData.suburbs?.filter((suburb: DatabaseLocationSnapshot) => 
        suburb.districtId === selectedDistrictId
      ) || [];
    } else if (selectedRegionId) {
      // Show districts in selected region
      return currentListingData.districts?.filter((district: DatabaseLocationSnapshot) => 
  ...
  const handleRegionChange = (regionId: string): void => {
    ...
    console.log('🗺️ Region changed to:', id, 
      availableDistricts.length > 0 ? `(${availableDistricts.length} districts available)` : '(no districts found)'
    );
  };
  ...
  const handleDistrictChange = (districtId: string): void => {
    ...
    const suburbsInDistrict = currentListingData?.suburbs?.filter((s: DatabaseLocationSnapshot) => s.districtId === id) || [];
    console.log('🏘️ District changed to:', id, `(${suburbsInDistrict.length} suburbs available)`);
  };
  ...
  const handleSort = (column: SortColumn): void => {
    ...
    console.log('🔄 Sort changed:', { column, order: sortOrder === 'asc' ? 'desc' : 'asc' });
  };

  // Sort and limit data
  const displayData = useMemo((): DatabaseLocationSnapshot[] => {
    const sorted = [...currentDisplayData].sort((a: DatabaseLocationSnapshot, b: DatabaseLocationSnapshot) => {
  ...
  const levelName: string = currentLevel.charAt(0).toUpperCase() + currentLevel.slice(1);

  // Get current location name for display
// In your PropertyDashboard component, replace the getCurrentLocationName function:
...
const getCurrentLocationName = (item: DatabaseLocationSnapshot): string => {
  ...
  const selectedRegionName: string | undefined = selectedRegionId ? 
    availableRegions.find((r: DropdownOption) => r.id === selectedRegionId)?.name : undefined;
  const selectedDistrictName: string | undefined = selectedDistrictId ? 
    availableDistricts.find((d: DropdownOption) => d.id === selectedDistrictId)?.name : undefined;
  ...
  if (!totals) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">No data available for {selectedListingType}. Make sure data collection is running.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-sm text-gray-500">
            {selectedListingType === 'HOUSES_TO_BUY' ? 'houses for sale' : 'houses for rent'}
          </p>
        </div>
        
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-medium text-gray-900">Current View</h3>
          <p className="text-3xl font-bold text-green-600">{displayData.length}</p>
          <p className="text-sm text-gray-500">{levelName}s showing</p>
        </div>
        
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-medium text-gray-900">Available</h3>
          <p className="text-3xl font-bold text-orange-600">{currentDisplayData.length}</p>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <div className="flex items-end">
            ...
            >
              {showAll ? `Showing All (${currentDisplayData.length})` : `Top 10 of ${currentDisplayData.length}`}
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center text-sm text-gray-600">
          ...
          <span className="ml-2">
            {selectedDistrictName ? `Suburbs in ${selectedDistrictName}` :
             selectedRegionName ? `Districts in ${selectedRegionName}` :
             `All regions`}
          </span>
                  ...
                  <tr key={item.id} className={index < 3 ? 'bg-yellow-50' : ''}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex items-center">
                        <span className="mr-2">
                          {((item.listingCount || 0) / totals.total * 100).toFixed(1)}%
                        </span>
                        <div className="w-16 bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-blue-600 h-2 rounded-full" 
                            style={{ 
                              width: `${((item.listingCount || 0) / totals.total * 100)}%` 
                            }}
                          />
                        </div>
                      </div>
      ...
      {/* Recent Collections */}
      <div className="bg-white p-6 rounded-lg shadow">
        <h3 className="text-xl font-bold text-gray-900 mb-4">Recent Data Collections</h3>
        <div className="space-y-3">
          {snapshots.slice(0, 5).map((snapshot: DatabaseWeeklySnapshot, index: number) => (
            <div key={snapshot.id} className={`flex justify-between items-center p-4 rounded-lg ${
